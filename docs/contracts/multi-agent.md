...
- every task carries an explicit `project_context: none | inherit` policy
  (ADR 0017), recorded on `agent.spawn` and distinct from
  `include_parent_canvas`. The default is `none`: child request assembly
  filters every project-context-classified item even when the parent canvas
  is included. `inherit` supplies the parent's exact frozen snapshot (one
  shared immutable pre-fan-out snapshot for parallel batches) even when
  `include_parent_canvas` is false, which controls only non-project parent
  items. A missing field on an older event decodes as `none`; an unknown
  value is invalid and never falls through to inheritance. Guardian
  `inherit` and coding-worker opt-ins are phase-5 scope; today every caller
  uses the `none` default.
