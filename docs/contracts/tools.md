...
| `code_swarm_review` | AgentSpawn | Session-level review gate: fans out the persisted CodeSwarm reviewer set in parallel over required explicit bounded `focus` context (up to 256 KiB) and returns per-reviewer findings for the calling agent to adjudicate. It never includes ambient session canvas. Optional: `personas`, `models` (non-empty one-off override; an empty model-facing list is omission), `max_tokens`. Advertised only in the root session when the `code-swarm` extension is wired and enabled; companions never see it (depth one). Config store, resolution chain, result shape, and failure honesty: multi-agent contract. |
