...
/// A test provider whose [`invoke`](ModelProvider::invoke) blocks until a test
/// explicitly opens its [`Gate`], turning "a turn is in flight" into a
/// deterministic state instead of a race against a background worker thread.
///
/// It replaces the `SlowEchoProvider` sleep-and-hope pattern (invoke sleeps
/// N ms and hopes the test thread wins): the worker parks inside `invoke`,
/// holding any shared state (steering queue, in-flight AppState) stable while
/// the test asserts, then the test calls [`Gate::open`] and drains to
/// completion — no timing assumptions, no sleeps.
///
/// The gate opens *permanently*, so every current and future invocation
/// proceeds once released; this makes it safe for tests that flush several
/// queued turns after a single `open()`.
#[derive(Debug)]
pub struct GateProvider<P = EchoProvider> {
    inner: P,
    gate: Gate,
}

impl GateProvider<EchoProvider> {
    /// Gate an [`EchoProvider`]. Returns the provider (to hand to the session)
    /// and the clonable [`Gate`] the test keeps to release it.
    pub fn echo() -> (Self, Gate) {
        Self::wrapping(EchoProvider)
    }
}

impl<P: ModelProvider> GateProvider<P> {
    /// Gate an arbitrary inner provider; the inner produces the response once
    /// the gate opens.
    pub fn wrapping(inner: P) -> (Self, Gate) {
        let gate = Gate::new();
        (
            Self {
                inner,
...
impl<P: ModelProvider> ModelProvider for GateProvider<P> {
    fn name(&self) -> &'static str {
        self.inner.name()
    }

    fn reasoning_effort(&self, model: &str) -> Option<&str> {
        self.inner.reasoning_effort(model)
    }

    fn set_resolved_secret_sink(&self, sink: ResolvedSecretSink) {
        self.inner.set_resolved_secret_sink(sink);
    }
...
/// Release handle for a [`GateProvider`]. Cloning shares the same underlying
/// gate, so the copy the provider holds and the copy the test keeps refer to
/// one latch.
#[derive(Clone, Debug)]
pub struct Gate {
...
impl Gate {
    ...
    /// Called by the gated provider: park until [`open`](Gate::open) is called.
    ...
    /// Permanently open the gate: the parked invocation and every later one
    /// proceed.
    pub fn open(&self) {
        let mut open = self.inner.open.lock().expect("gate mutex poisoned");
        *open = true;
...
mod tests {
    ...
    #[test]
    fn gate_provider_blocks_invoke_until_opened() {
        ...
        let worker = {
            ...
            std::thread::spawn(move || {
                let stream = provider.invoke(empty_request()).expect("gated invoke");
                // Drain so the echo actually runs.
                let _ = stream.count();
                done.store(true, std::sync::atomic::Ordering::SeqCst);
            })
        };

        // The worker is parked inside `invoke`; it must not have finished.
    ...
    #[test]
    fn gate_provider_open_lets_every_invocation_through() {
        // Opening is permanent: an invoke that arrives after `open` never parks.
        let (provider, gate) = GateProvider::echo();
        gate.open();
        let stream = provider.invoke(empty_request()).expect("gated invoke");
