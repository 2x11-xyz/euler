...
pub(crate) fn resolve_live_linked_observer(
    id: &str,
    cadence_override: Option<std::num::NonZeroU64>,
) -> Result<
    Option<(
        euler_core::RoundObserverConfig,
        std::sync::Arc<dyn euler_sdk::Extension>,
    )>,
> {
    let registry = extension_registry()?;
    let Some(linked) = linked_extension(&registry, id)? else {
    ...
    reject_bundled_link_collision(id)?;
    let observer = linked.descriptor.observer.clone().ok_or_else(|| {
        anyhow!("--observe {id} is not supported: extension {id} declares no observer command pair")
    })?;
    let package = load_enabled_linked_process(&registry, &linked)?;
    let extension = ManagedProcessExtension::from_package(&package)
        .map_err(|error| anyhow!(error.to_string()))?;
    let cadence_rounds = match cadence_override {
        Some(cadence) => cadence,
        None => std::num::NonZeroU64::new(observer.default_cadence_rounds)
            .expect("validated observer cadence must be non-zero"),
    };
    Ok(Some((
        euler_core::RoundObserverConfig {
            cadence_rounds,
