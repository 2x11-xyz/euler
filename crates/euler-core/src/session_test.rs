...
/// Perf guard (deep review P3-a): each prepared model request folds the
/// event stream a bounded, pinned number of times, and the count is
/// per-round constant — R rounds cost R times one round's passes, never a
/// growing multiple. Per round: the project-context fold (1, threaded into
/// canvas assembly rather than folded again inside it), the canvas
/// assembly passes (active swap, tool pairs, included model calls, context
/// slots, item projection = 5), and the unseen-instructions scan (1).
#[test]
fn prepare_model_request_full_stream_pass_count_is_pinned() {
    let temp = tempfile::tempdir().expect("temp dir");
    std::fs::write(temp.path().join("a.txt"), "alpha").expect("a");
    let provider = ScriptedProvider::new(vec![
        FixtureResponse::Assistant("hi".to_owned()),
        FixtureResponse::ToolCalls(vec![euler_provider::ToolCall {
            id: "call-a".to_owned(),
            name: "read_file".to_owned(),
            input: json!({"path": "a.txt"}),
    ...
    crate::canvas::reset_full_stream_passes();
    session.run_turn("hello").expect("first turn");
    assert_eq!(
        crate::canvas::full_stream_passes(),
        7,
        "one round prepares one request: 7 full-stream passes"
    );

    crate::canvas::reset_full_stream_passes();
    session.run_turn("read it").expect("second turn");
    assert_eq!(
        crate::canvas::full_stream_passes(),
        14,
        "two rounds prepare two requests: exactly twice one round's passes"
    );
}
