...
const MATRIX: &[Case] = &[
    ...
    Case { name: "shell / ask / static-safe precedes grant coverage (attribution)",
        capability: ShellExec, mode: DefaultMode, grants: &[(Store::Session, "ls")],
        tool: "run_shell", subject: "ls -la", expected: StaticSafe },
    ...
    // Prerequisite of the #217 split (audit F01, F02, F34): the permissive
    // danger walk vetoes every auto-approval path, and the prove-safe
    // grammar no longer proves write operands or shell-expanded words.
    Case { name: "shell / ask / dangerous command is never covered by a grant",
        capability: ShellExec, mode: DefaultMode, grants: &[(Store::Session, "rm")],
        tool: "run_shell", subject: "rm -f scratch.txt", expected: Asks },
    Case { name: "shell / ask / dangerous command inside a wrapper is never covered",
        capability: ShellExec, mode: DefaultMode, grants: &[(Store::Session, "sh")],
        tool: "run_shell", subject: "sh -c 'rm -rf .'", expected: Asks },
    Case { name: "shell / ask / dangerous command under an unscoped grant still asks",
        capability: ShellExec, mode: Set(Ask), grants: &[(Store::Session, "")],
        tool: "run_shell", subject: "ls | xargs rm -f", expected: Asks },
    Case { name: "shell / ask / uniq output operand asks (audit F01)",
        capability: ShellExec, mode: DefaultMode, grants: &[],
        tool: "run_shell", subject: "uniq input.txt output.txt", expected: Asks },
    Case { name: "shell / ask / unquoted glob asks (audit F02)",
        capability: ShellExec, mode: DefaultMode, grants: &[],
        tool: "run_shell", subject: "cat *.txt", expected: Asks },
    Case { name: "shell / ask / cd compound asks (audit F02)",
        capability: ShellExec, mode: DefaultMode, grants: &[],
        tool: "run_shell", subject: "cd src && ls", expected: Asks },
    Case { name: "shell / ask / git metadata read asks (audit F34)",
        capability: ShellExec, mode: DefaultMode, grants: &[],
        tool: "run_shell", subject: "cat .git/config", expected: Asks },
    Case { name: "shell / ask / proven wrapper script runs without a prompt",
        capability: ShellExec, mode: DefaultMode, grants: &[],
        tool: "run_shell", subject: "sh -c 'ls -la'", expected: StaticSafe },
    ...
    Case { name: "shell / session-allow / ordinary command is still mode-allowed",
        capability: ShellExec, mode: Set(SessionAllow), grants: &[],
        tool: "run_shell", subject: "touch mode-allowed", expected: AllowedByMode },
    Case { name: "shell / always-deny / dangerous command is denied, not prompted",
        capability: ShellExec, mode: Set(AlwaysDeny), grants: &[],
        tool: "run_shell", subject: "rm -rf scratch", expected: Denied },
